...
Release tagging script for `delphi-dccbuild`.
...
- [ ] `$script:Version` in `source/delphi-dccbuild.ps1` has been updated to `X.Y.Z`
...
1. `$script:Version` in `source/delphi-dccbuild.ps1` matches the `-Version` argument
...
delphi-dccbuild  tag-release
============================
  Version : 1.0.0
  Tag     : v1.0.0
  Repo    : C:\dev\delphi-dccbuild
...
```text
FAIL  $script:Version in delphi-dccbuild.ps1 is '1.0.0' but -Version arg is '1.1.0'.
      Update $script:Version in the script and commit before tagging.
```

Update `$script:Version = '1.1.0'` in `source/delphi-dccbuild.ps1`, commit, push, then re-run.
...
```text
  ...
  ok  git found (git version 2.47.0.windows.1)
  ok  inside git repository
  warn  Not on 'main' (on 'hotfix/dcc-path'); -SkipBranchCheck override active
  ok  working tree is clean
  ok  origin remote found
  ok  tags fetched
  ok  HEAD is up-to-date with origin/main
