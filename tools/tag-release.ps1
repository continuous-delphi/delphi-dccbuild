...
# Creates and pushes a vX.Y.Z release tag for delphi-dccbuild.
...
#   - $script:Version in delphi-dccbuild.ps1 matches the Version argument
...
$scriptFile    = Join-Path $repoRoot 'source' 'delphi-dccbuild.ps1'
$changelogFile = Join-Path $repoRoot 'CHANGELOG.md'
$tag           = "v$Version"

Write-Host ""
Write-Host "delphi-dccbuild  tag-release" -ForegroundColor White
...
if ($scriptContent -notmatch '\$script:Version\s*=\s*''([^'']+)''') {
  Fail "Could not find '`$script:Version = ''...''' in delphi-dccbuild.ps1."
}

$scriptVersion = $Matches[1]
if ($scriptVersion -ne $Version) {
  Fail "`$script:Version in delphi-dccbuild.ps1 is '$scriptVersion' but -Version arg is '$Version'.`n       Update `$script:Version in the script and commit before tagging."
}
