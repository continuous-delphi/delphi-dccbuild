...
<#
.SYNOPSIS
  Smoke test -- quick green-flag check that delphi-dccbuild.ps1 is present
...
.DESCRIPTION
  ...
  It is not a substitute for the unit tests in delphi-dccbuild.Tests.ps1.
#>

Describe 'delphi-dccbuild.ps1 smoke test' {
  ...
  It 'Invoke-DccProject function is available after dot-sourcing' {
    (Get-Command -Name Invoke-DccProject -CommandType Function -ErrorAction SilentlyContinue) |
      Should -Not -BeNull
  }

  It 'Get-CompilerName function is available after dot-sourcing' {
    (Get-Command -Name Get-CompilerName -CommandType Function -ErrorAction SilentlyContinue) |
      Should -Not -BeNull
  }
