...
<#
.SYNOPSIS
  PSScriptAnalyzer lint test for source/delphi-dccbuild.ps1

.DESCRIPTION
  Runs Invoke-ScriptAnalyzer against delphi-dccbuild.ps1 using the default
  rule set and asserts zero violations.  On failure each violation is listed
  with its rule name, severity, and line number.
#>

Describe 'PSScriptAnalyzer - delphi-dccbuild.ps1' {
